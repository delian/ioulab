...
function restPutIous(req, res) {
	...
	// Now we do have in model a reference to the original type
	if (model.minReq) {
		if (typeof model.minReq.memory != 'undefined') req.body.memory=Math.max(model.minReq.memory,req.body.memory);
		if (typeof model.minReq.nvram != 'undefined') req.body.nvram=Math.max(model.minReq.nvram,req.body.nvram);
		if (typeof model.minReq.ethernet != 'undefined') req.body.ethernet=Math.max(model.minReq.ethernet,req.body.ethernet);
		if (typeof model.minReq.serial != 'undefined') req.body.serial=Math.max(model.minReq.serial,req.body.serial);
	}
	if (model.maxReq) {
		if (typeof model.maxReq.memory != 'undefined') req.body.memory=Math.min(model.maxReq.memory,req.body.memory);
		if (typeof model.maxReq.nvram != 'undefined') req.body.nvram=Math.min(model.maxReq.nvram,req.body.nvram);
		if (typeof model.maxReq.ethernet != 'undefined') req.body.ethernet=Math.min(model.maxReq.ethernet,req.body.ethernet);
		if (typeof model.maxReq.serial != 'undefined') req.body.serial=Math.min(model.maxReq.serial,req.body.serial);
	}
