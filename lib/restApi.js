...
function restPutLabDevices(req, res) {
	...
	colDevices.findAndModify({ id: parseInt(req.params.id) }, [], { $set: { name: req.body.name } }, { safe: true }, function(err,q) {
...
function restPutDeviceLinks(req, res) {
	...
	colLinks.findAndModify({ id: parseInt(req.params.id) },[],{ $set: {'name': req.body.name } }, { safe: true }, function(err,q) {
		q.name=req.body.name;
		e.emit('updateLink',q);
		return res.send(200,q);
	});
