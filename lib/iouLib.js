...
function qemuCmdBuild(bin, id, mem, params, image) {
	...
	var vncId=100+id;
	return [ 'bash', '-c', bin+' -m '+mem+'M -device e1000,netdev=mynet,mac='+mac+' -netdev socket,id=mynet,listen=127.0.0.1:'+netPort+' -display vnc=127.0.0.1:'+vncId+' '+params+' '+image ]; // Remove the configuration file as it is not used
}
...
function openQEMU(cnf) {
	...
	function startTheNet() {
		...
		colLinks.findOne({
			...
			sNet.on('data',function(msg){
				...
				try {
					s.sendto(data,0,data.length,dstPath,function() {
						// Data is sent
					});
				} catch(e) {
					return startNet();
	...
	function startNet() {
		if (netTimer) clearTimer(netTimer);
		try {
			startTheNet();
		} catch(e) {
				...
				colDevices.findOne({ id: parseInt(deviceId) },function(err,device){
					...
					colIous.findOne({ id: parseInt(device.type) },function(err,ious){
						...
						if (iouBin.type=='qemu') {
							...
							var imgName = config.labPath+"/"+deviceId.toString()+'.img';
							if (!iouBin.qemuImage) doQemuStart("");
							else {
							   try {
								if (fs.statSync(imgName)) {
									console.log('Image existed, start Qemu');
									doQemuStart(imgName);
								}
							   } catch(e) {
								console.log('Copy ',iouBin.qemuImage,imgName);
								copyFile(iouBin.qemuImage,imgName,function() {
									doQemuStart(imgName);
								});
							   }
							}
