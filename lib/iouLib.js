...
function qemuCmdBuild(bin, id, mem, params, image) {
	...
	var vncId=100+id;
	return [ 'bash', '-c', bin+' -m '+parseInt(mem).toString()+'M -device pcnet,netdev=mynet,mac='+mac+' -netdev socket,id=mynet,listen=127.0.0.1:'+netPort+' -display vnc=127.0.0.1:'+vncId+' '+params+' '+image ]; // Remove the configuration file as it is not used
}
