...
function qemuCmdBuild(bin, id, mem, image) {
	console.log(arguments.callee,arguments);
	mem = (typeof mem == 'undefined') ? 256 : Math.max(128,mem);
	image = image?image:"";
	var mac="AA:BB:CC:"+("0000"+2).slice(-4).replace(/(..)(..)/,'$1:$2')+":00";
	var netPort=40000+id;
	var vncId=100+id;
	return [ 'bash', '-c', bin+' -m '+mem+'M -device e1000,netdev=mynet,mac='+mac+' -netdev socket,id=mynet,listen=127.0.0.1:'+netPort+' -display vnc=127.0.0.1:'+vncId+' '+image ]; // Remove the configuration file as it is not used
}
...
function openQEMU(cnf) {
	...
	var term;
	var iouObj = { status: 0, cnf: cnf, buff: "", id: cnf.deviceId };
	var iouPath = qemuCmdBuild(cnf.iouBin, cnf.deviceId, cnf.memory, cnf.nvram, cnf.confName, cnf.ethernet, cnf.serial);
	
	var qemuPort = 40000+cnf.deviceId;
