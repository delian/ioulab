...
function iouObject(db, e) {
	...
	function uiUpdateLink(msg) {
		...
		if (msg.source.id && (!msg.source.name)) {
			if (msg.type == 'serial')
				intApi.iouFreeSerial(msg.source.id, updSource);
			else
				intApi.iouFreeEthernet(msg.source.id, updSource);
		}
		...
		if (msg.target.id && (!msg.target.name)) {
			if (msg.type == 'serial')
				intApi.iouFreeSerial(msg.target.id, updTarget);
			else
				intApi.iouFreeEthernet(msg.target.id, updTarget);
		}
	...
	e.on('resetDeviceFile',function(msg) {
	...
	e.on('saveDeviceFile',function(msg) {
		...
		if (data) {
			colNvFiles.update({ id: parseInt(msg.id)},{ id: parseInt(msg.id), data: data },{ upsert:true, safe:false}, function(err,q){
