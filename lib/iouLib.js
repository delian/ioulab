...
function iouObject(db, e) {
				...
				colDevices.findOne({ id: parseInt(deviceId) },function(err,device){
					...
					colIous.findOne({ id: parseInt(device.type) },function(err,ious){
						...
						colNvFiles.findOne({ id: parseInt(deviceId) }, function(err,q) {
							if (q) {
								try {
									fs.statSync(nvname(deviceId));
								} catch(e) {
									fs.fileWriteSync(nvname(deviceId),q.data); // Restore the data, if we have it in the DB but not out
								}
							}
	...
	e.on('resetDeviceFile',function(msg) {
		...
		// TODO: Now we shall add the default config file and nvram whatever it is
	});
