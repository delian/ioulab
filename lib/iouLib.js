...
function openIOU(cnf) {
	...
	function iouOpen() {
		console.log(arguments.callee);
		var term = appSpawn('bash' || cnf.iou || iouPath || 'bash', cnf.dir);
		term.on('data', function(msg) {
			console.log('>>>', arguments[0]);
			if (cnf.data) cnf.data(msg);  // Call back the data
			iouObj.buff=(iouObj.buff+msg).toString().substr(0,maxBuffChars); // Keep a buffer, for initial connection
		});
...
function iouObject(db, e) {
	...
	// TODO: Implement terminal events handling for the IOU
	...
	// TODO: Cancel the CTRL+C for the IOU (probably start it trough wrapper)
	
	function iouStart(deviceId, cb) {
		console.log(arguments.callee);
		if (iObj[deviceId] && (!iObj[deviceId].isStarted())) {
	...
	e.on('terminalConnect',function(msg) {
		...
		e.emit('terminalOut',{ id:msg.id, data:obj[msg.id].buff});
	});
