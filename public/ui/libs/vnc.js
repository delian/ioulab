(function () {
  ...
  Screen.prototype.copyRect = function(rect) {
    //console.log('copy image',rect);
    var img = this._context.getImageData(rect.srcx,rect.srcy,rect.width,rect.height);
    this._context.putImageData(img,rect.dstx,rect.dsty);
  }
  ...
  Screen.prototype.addMouseHandler = function (cb) {
    ...
    this._canvas.addEventListener('mousedown', function (e) {
      ...
      self.mouseButton = e.which;
      cb.call(null, e.pageX, e.pageY, bMask[e.which]);
      e.preventDefault();
    }, false);
    ...
    this._canvas.addEventListener('mousemove', function (e) {
      console.log('move',e);
      cb.call(null, e.pageX, e.pageY, bMask[self.mouseButton||0]);
      e.preventDefault();
    }, false);
  };
  ...
  Screen.prototype.addKeyboardHandlers = function (cb) {
    document.addEventListener('keydown', function (e) {
      if (e.ctrlKey) e.keyCode = e.keyCode & 0x1F;
...
//      password: config.password
  ...
  Client.prototype._addHandlers = function (callback) {
    ...
    this._socket.on('framecopy', function(frame) {
      self._screen.copyRect(frame);
    });
  };
  ...
  var canvas = document.getElementById('screen'), screen = new Screen(canvas), client = new Client(screen);
